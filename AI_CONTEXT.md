...
2. **Stack Tecnológico Multilenguaje OBLIGATORIO**:
   ...
   - **Python (AI Stems Engine / ONNX Export)**: Lenguaje oficial para la creación, cuantización (INT8/FP16) y prueba de modelos de **separación de 4 Stems (Voces, Batería, Bajo, Melodía/Teclados)** compatibles con ONNX Runtime v2 en Android (Mobile-UNet 4-Stem HD 18.5MB) y motor de Auto-Masterizado.
...
- El Motor de Búsqueda Difusa utiliza `RustFuzzySearchEngine` para calcular distancias Levenshtein en tiempo real permitiendo búsquedas tolerantes a erratas.
- La extracción de carátulas incrustadas en formato APIC / PICTURE prioritario se canaliza a través de `RustMetadataParser` hacia `AudioImporter`.
- La función Video a Música + Canvas de Fondo utiliza `importVideoAsTrackAndCanvas` ejecutado en `Dispatchers.IO` con copiado por buffers de 64KB y modal de progreso `VideoImportProgressModal` para procesar videos largos de alta definición (4K) sin congelar la UI.
- El Descargador del Modelo IA ONNX HD de 18.5 MB utiliza `StemModelManager` para verificar y descargar el archivo `mobile_unet_4stems_hd.onnx` de forma asíncrona desde un servidor CDN de Cloudflare Pages con barra de progreso.
- La Cola de Reproducción Interactiva estilo Lark se gestiona con `QueueController` y `QueueModalSheet`, calculando y marcando automáticamente la pista siguiente ("A CONTINUACIÓN") con reordenamiento y eliminación de pistas.
- El Simulador de Ambientes Reverb 3D (Spatial HRTF Audio) se procesa mediante `SpatialReverbEnvironment` y `PresetReverb` en `AudioDspEngine` con opciones para Habitación Pequeña, Salón Mediano, Gran Salón, Catedral y Estadio.

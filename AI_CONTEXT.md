...
2. **Stack Tecnológico Multilenguaje OBLIGATORIO**:
   ...
   - **Python (Módulo Nativo `python_ai/`)**: Módulo nativo preservado en `python_ai/` con scripts de procesamiento y preparación de modelos de audio ONNX/TFLite para desarrollos futuros.
...
- Sincroniza en tiempo real los estados de favorito (`isFavorite`) e icono de corazón con cambio de color animado (`animateColorAsState`) entre el reproductor a pantalla completa (`PlayerFullScreen` con `PlayerBackgroundLayer`), mini reproductor (`MiniPlayer`) y las listas.
...
- Utiliza `LrcParser` para parsear letras sincronizadas con marcas de tiempo `[mm:ss.xx]` y `PlayerLyricsSection` / `PlayerLyricsView` para auto-scroll y resaltado en verde de la frase activa según el tiempo actual.
...
- Utiliza `AudioImporter` y `TrackMetadataExtractor` (con `RustMetadataParser`) al importar o inspeccionar metadatos de archivos de audio.
- La Mejora de Audio 3D Espacial se gestiona mediante `AudioDspEngine` y `AudioFxManager` apoyado por sub-controladores (`EqualizerFxController`, `Spatial3dAudioFxController`, `ReverbFxController`), `Audio3dSpeakerMode` (`SINGLE_SPEAKER`, `DUAL_SPEAKER`, `HEADPHONES`) y `PlayerAudio3dEnhancerView`.
- Los parámetros de velocidad y tono se aplican de forma segura mediante `PlaybackParamsController`.
...
- El Panel de Volumen Flotante Personalizado utiliza `VolumeController` e `CustomVolumePanelHUD` para interceptar las teclas físicas de volumen del hardware en `MainActivity`.
- El Temporizador de Sueño Inteligente se gestiona en `MusicPlayerManager` (con reducción progresiva fade-out de 15s de volumen) y la hoja modal `SleepTimerSheet`.
- El Editor Manual de Etiquetas ID3 y Portadas utiliza `EditTrackMetadataDialog` e integra con `LibraryDelegate` para actualizar título, artista, álbum, género, mood y portadas WebP.
- La vista del reproductor a pantalla completa desglosa sub-elementos en `PlayerBackgroundLayer`, `PlayerHeaderArtSection` y `PlayerLyricsSection`.
...
- El soporte para "Abrir com..." desde exploradores de archivos externos se procesa mediante `handleIncomingAudioIntent(intent)` en `MainActivity`.
- El Motor de Búsqueda Difusa utiliza `RustFuzzySearchEngine` para calcular distancias Levenshtein en tiempo real.
- La función Video a Música + Canvas de Fondo utiliza `importVideoAsTrackAndCanvas` ejecutado en `Dispatchers.IO` con copiado por buffers de 64KB y modal de progreso `VideoImportProgressModal`.
- La Cola de Reproducción Interactiva estilo Lark se gestiona con `QueueController` y `QueueModalSheet`.

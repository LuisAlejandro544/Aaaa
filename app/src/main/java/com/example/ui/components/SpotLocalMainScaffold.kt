...
fun SpotLocalMainScaffold(
    ...
    onLaunchVideoImportPicker: () -> Unit = {},
    ...
    val reverbEnvironment by viewModel.reverbEnvironment.collectAsStateWithLifecycle()
    val reverbStrength by viewModel.reverbStrength.collectAsStateWithLifecycle()
    ...
    val isImportingVideo by viewModel.isImportingVideo.collectAsStateWithLifecycle()
    ...
    Box(
                    ...
                    when (currentTab) {
                        SpotifyTab.HOME -> {
                            HomeScreen(
                                ...
                                onImportVideoClick = onLaunchVideoImportPicker,
        ...
        AnimatedVisibility(
            ...
            PlayerFullScreen(
                ...
                onPlayQueueIndex = { idx -> viewModel.playQueueIndex(idx) },
                ...
                onReverbEnvironmentSelect = { env -> viewModel.setReverbEnvironment(env) },
                onReverbStrengthChange = { strength -> viewModel.setReverbStrength(strength) },
        ...
        trackOptionsTarget?.let { track ->
            TrackOptionsDialog(
                ...
                onClassifyMoodAndGenre = { viewModel.classifyTrackMoodAndGenre(it) },
                onDeleteTrack = { viewModel.deleteTrack(it) },
                onPickCustomCover = { onPickCustomCover(it) },
                onPickCanvasVideo = { onPickCanvasVideo(it) },
                onRemoveCanvasVideo = { viewModel.removeCanvasVideo(it) }
            )
        }
        ...
        // Fullscreen Loading Overlay for Video to Music + Canvas Processing
        if (isImportingVideo) {
            com.example.ui.components.player.VideoImportProgressModal(
                progressText = importVideoProgressText
            )
        }

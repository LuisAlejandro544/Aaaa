...
object AudioFingerprintEngine {
    ...
    fun computeFingerprintHash(fileSizeBytes: Long, durationMs: Long, title: String = "", artist: String = ""): String {
        val normalizedTitle = title.lowercase().replace(Regex("[^a-z0-9]"), "")
        val normalizedArtist = artist.lowercase().replace(Regex("[^a-z0-9]"), "")
        val rawInput = "raw_fp:$fileSizeBytes:$durationMs:$normalizedTitle:$normalizedArtist"
        return try {

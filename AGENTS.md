...
- Descargador de los 4 Modelos IA TFLite FP16 (Voces UVR MDX, Bajo, Batería y Melodía Kuielab, ~75 MB total / 74.9 MB) gestionado por `StemModelManager` desde GitHub Release v1.0 (`https://github.com/LuisAlejandro544/Modelos/releases/tag/v1.0`), con diálogo interactivo al ingresar a la app (`ModelDownloadPromptDialog`), progreso en segundo plano y enlace directo a la release para máxima transparencia.
- Ejecutor de IA optimizado para TensorFlow Lite (`TfliteInferenceRunner`) con aceleración por GPU/NNAPI y pipeline decodificador de audio (`AudioDecoderPipeline`) mediante `MediaCodec` y espectrogramas STFT/iSTFT para procesar cualquier formato (MP3, WAV, FLAC, AAC, OGG).
